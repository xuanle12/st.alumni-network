...
.register-page{
  ...
  padding:32px 16px;animation:reg-fadeUp .4s ease both;
}

.reg-card{
  width:100%;max-width:480px;background:var(--card);
  border-radius:16px;border:1px solid var(--border);
  padding:40px 36px;box-shadow:var(--shadow);position:relative;
}
...
.reg-back a{
  ...
  color:#64748b;text-decoration:none;font-size:14px;transition:.15s;background:#fff;
}
.reg-back a:hover{background:#f8fafc;color:#0f172a;}

.reg-head{text-align:center;margin-bottom:24px;}
.reg-logos{display:flex;align-items:center;justify-content:center;gap:20px;margin-bottom:14px;}
.reg-logos img{width:60px;height:60px;object-fit:contain;border-radius:8px;}
.reg-head-title{font-size:16px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:#0f172a;line-height:1.25;margin-bottom:3px;}
.reg-head-sub{font-size:12.5px;font-weight:500;color:#64748b;text-transform:uppercase;letter-spacing:.3px;}
...
.reg-label{display:block;font-size:12.5px;font-weight:600;color:#374151;margin-bottom:6px;letter-spacing:.2px;}
  ...
  <div class="reg-card">
    ...
    <div class="reg-row">
      <div class="reg-field">
        ...
        <input class="reg-input @error('ho') is-error @enderror"
          wire:model="ho" type="text" placeholder="Nguyễn Văn">
      ...
      <div class="reg-field">
        ...
        <input class="reg-input @error('ten') is-error @enderror"
          wire:model="ten" type="text" placeholder="An">
    ...
    <div class="reg-field">
      ...
      <input class="reg-input @error('email') is-error @enderror"
        wire:model="email" type="email" placeholder="ten@gmail.com">
    ...
    <div class="reg-row">
      ...
      <div class="reg-field">
        ...
        <input class="reg-input"
          wire:model="password_confirmation" type="password" placeholder="••••••••">
      </div>
    </div>
    ...
    <div class="reg-verify-box" x-data="{ open: false }">
      ...
      <div x-show="open" x-transition>
        <p class="reg-verify-desc">
          Điền ít nhất <strong>2 trong 5 thông tin</strong> khớp với danh sách cựu sinh viên để được cấp quyền và kích hoạt tài khoản ngay. Thông tin sẽ được kiểm tra khi bấm <em>Đăng ký</em>.
        </p>

        <div class="reg-row">
          <div class="reg-field">
            <label class="reg-label">Họ tên đầy đủ</label>
            <input type="text" class="reg-input" wire:model="full_name" placeholder="Nguyễn Văn A">
          </div>
          <div class="reg-field">
            <label class="reg-label">Mã sinh viên</label>
            <input type="text" class="reg-input" wire:model="student_code" placeholder="641234">
          </div>
        </div>
        ...
        <div class="reg-row">
          <div class="reg-field">
            ...
            <input type="text" class="reg-input" wire:model="class_name" placeholder="K64CNTTA">
          </div>
          <div class="reg-field">
            <label class="reg-label">Năm tốt nghiệp</label>
            <input type="number" class="reg-input" wire:model="graduation_year"
              placeholder="2024" min="1990" max="{{ date('Y') }}">
          </div>
        </div>

        <div class="reg-field">
          <label class="reg-label">Ngành học</label>
          <input type="text" class="reg-input" wire:model="major" placeholder="Công nghệ thông tin">
        </div>
      </div>
    </div>
    {{-- ── /VERIFY BOX ── --}}
    ...
    <a href="{{ route('sso.login') }}" class="reg-sso">
