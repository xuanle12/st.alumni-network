...
.lok{
    background:#eff6ff;border:1px solid #93c5fd;color:#063a68;
    border-radius:10px;padding:10px 14px;font-size:13px;margin-bottom:16px;
    display:flex;align-items:center;gap:8px;
}
...
.linput:focus{border-color:#0961aa;box-shadow:0 0 0 3px rgba(9,97,170,.1);}
.linput::placeholder{color:#94a3b8;}
.lfield-err{font-size:12px;color:#dc2626;margin-top:4px;display:block;}

.lbtn-submit{
    width:100%;padding:13px;background:#0961aa;color:#fff;border:none;
...
.lbtn-submit:hover:not(:disabled){background:#0c83d8;transform:translateY(-1px);}
...
.lregister a{color:#0961aa;font-weight:600;text-decoration:none;}
...
<div class="lp">
  <div class="lcard">
    ...
    <div class="lcard-head">
      ...
      <div class="lcard-title">Quên mật khẩu</div>
      <div class="lcard-sub">Nhập email để nhận liên kết đặt lại mật khẩu</div>
    </div>

    @if($sent)
      <div class="lok">
        <i class="fa-solid fa-circle-check"></i>
        Nếu email tồn tại trong hệ thống, một liên kết đặt lại mật khẩu đã được gửi tới <strong>{{ $email }}</strong>. Vui lòng kiểm tra hộp thư (kể cả mục Spam).
      </div>
      <div class="lregister">
        <a href="{{ route('login') }}" wire:navigate>Quay lại đăng nhập</a>
      </div>
    @else
      @error('email')
      ...
      <form wire:submit="sendResetLink" class="lform">
        <div class="lfield">
        ...
        <button type="submit" class="lbtn-submit" wire:loading.attr="disabled">
          <span wire:loading.remove>Gửi liên kết đặt lại mật khẩu</span>
