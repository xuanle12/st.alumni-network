...
<div class="jf-wrap">
  ...
  <div class="jf-card">
    ...
    <div class="fg">
      <div class="fi full">
        <label>Tên sự kiện<span>*</span></label>
        <input wire:model="title" type="text" placeholder="VD: Ngày hội việc làm VNUA 2026, Hội thảo khởi nghiệp...">
        @error('title')<div class="err"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</div>@enderror
      </div>
      <div class="fi">
        <label>Đơn vị tổ chức<span>*</span></label>
        <input wire:model="organizer" type="text" placeholder="Tên đơn vị / CLB / cá nhân tổ chức">
        @error('organizer')<div class="err"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</div>@enderror
      </div>
      <div class="fi">
        <label>Email liên hệ<span>*</span></label>
        <input wire:model="contact_email" type="email" placeholder="lienhe@example.com">
        @error('contact_email')<div class="err"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</div>@enderror
      </div>
      <div class="fi">
        <label>Địa điểm</label>
        <input wire:model="location" type="text" placeholder="VD: Hội trường A, Học viện Nông nghiệp...">
      </div>
      <div class="fi">
        <label>Hình thức tham gia</label>
        <select wire:model="badge">
          <option value="free">Miễn phí</option>
          <option value="register">Cần đăng ký</option>
          <option value="paid">Có phí</option>
        </select>
      </div>
    </div>

    <div class="jf-section">Thời gian</div>
    <div class="fg">
      <div class="fi">
        <label>Ngày diễn ra<span>*</span></label>
        <input wire:model="event_date" type="date" min="{{ date('Y-m-d') }}">
        @error('event_date')<div class="err"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</div>@enderror
      </div>
      <div class="fi">
        <label>Giờ bắt đầu</label>
        <input wire:model="start_time" type="time">
        @error('start_time')<div class="err"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</div>@enderror
      </div>
      <div class="fi">
        <label>Giờ kết thúc</label>
        <input wire:model="end_time" type="time">
        @error('end_time')<div class="err"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</div>@enderror
      </div>
    </div>

    <div class="jf-section">Mô tả sự kiện</div>
    <div class="fg">
      <div class="fi full">
        <textarea wire:model="description" placeholder="Nội dung, chương trình, đối tượng tham gia, quyền lợi..."></textarea>
