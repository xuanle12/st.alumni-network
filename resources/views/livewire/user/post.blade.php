...
.ic-b{background:#eff6ff}.ic-g{background:#eff6ff}.ic-o{background:#fff7ed}.ic-a{background:#fffbeb}
...
.bd-g{background:#eff6ff;color:#075490}.bd-g::before{background:#0961aa}
...
<x-table minWidth="680px">
  ...
  <tr>
    ...
    <td><x-badge :color="$bColor">{{ $bl }}</x-badge></td>
    ...
    <td style="text-align:right">
      <x-table.action-btn>
        <div class="adm-dd-item" wire:click="openView({{ $post->id }})">
          <i class="fa-solid fa-eye adm-dd-ic"></i> Xem trước
        </div>

        @if($post->status === 'pending')
        <div class="adm-dd-item green" wire:click="openApprove({{ $post->id }})">
          <i class="fa-solid fa-check adm-dd-ic"></i> Duyệt bài
        </div>
        @endif

        @if(in_array($post->status, ['draft', 'hidden']))
        <div class="adm-dd-item green" wire:click="openApprove({{ $post->id }})">
          <i class="fa-solid fa-check adm-dd-ic"></i> Duyệt & Đăng
        </div>
        @endif

        @if($post->status === 'published')
        <div class="adm-dd-item amber" wire:click="toDraft({{ $post->id }})">
          <i class="fa-solid fa-clock adm-dd-ic"></i> Về bản nháp
        </div>
        <div class="adm-dd-item amber" wire:click="hidePost({{ $post->id }})">
          <i class="fa-solid fa-eye-slash adm-dd-ic"></i> Ẩn bài
        </div>
        @endif
        ...
        <div class="adm-dd-item" wire:click="openEdit({{ $post->id }})">
          <i class="fa-solid fa-edit adm-dd-ic"></i> Chỉnh sửa
        </div>

        <div class="adm-dd-sep"></div>

        <div class="adm-dd-item red" wire:click="confirmDelete({{ $post->id }})">
          <i class="fa-solid fa-trash adm-dd-ic"></i> Xoá
        </div>
      </x-table.action-btn>
    </td>
  </tr>
  ...
  <div class="mo mo-sm">
    ...
    <div class="mo-body">
      ...
      <p style="font-size:13px;color:#64748b;line-height:1.7">
        Bạn có chắc muốn <strong style="color:#075490">duyệt và đăng</strong> bài viết này không?<br>
